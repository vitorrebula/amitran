...
function App() {
  ...
  const buscaVeiculos = async () => {
    try {
      const response = await axios.get('http://192.168.0.149:8080/veiculo');
  ...
  const buscaFuncionarios = async () => {
    try {
      const response = await axios.get('http://192.168.0.149:8080/Funcionario');
  ...
  const buscaServicosPorFaixaDeData = async (data: string) => {
    try {
      const response = await axios.get<Servico[]>(`http://192.168.0.149:8080/servico/data/${data}`);
